...
Node.prototype.visit = function(parent) {
  ...
  // stroke(this.value,100,255);
  // line(parent.x, parent.y, this.x, this.y);
  // Draw a circle
  noStroke();
  // colorMode(HSB, 100);
  // fill(this.value[0],this.value[1],this.value[2],this.value[3]);
  fill(this.value);
...
Node.prototype.addNode = function(n) {
  ...
  } else if (n.value[0] > this.value[0]) {
    // Is there nothing there? Place the node
    if (this.right == null) {
      this.right = n;
      this.right.x = this.x + (width / pow(2, n.distance));
      this.right.y = this.y + (height / 30);
    // Keep going!
    } else {
      n.distance++;
